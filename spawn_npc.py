...
def main():
    ...
    argparser.add_argument(
        ...
        type=int,
        default=3004,
        help='UDP port to recieve kill command from (default: 3004)')
    argparser.add_argument(
        '--sync',
        action='store_true',
    ...
    try:
        ...
        def kill_thread(sock):
            while True:
                recData = None
                try:
                    recData = sock.recv(1024) #1024 buffer size
                    if (recData != None): 
                        if recData == b"kill":
        ...
        pasta_rec_sock.bind(("127.0.0.1", args.kill_port))
        pasta_rec_sock.settimeout(1)
        t = threading.Thread(target = kill_thread, args = [pasta_rec_sock])
        t.setDaemon(True) # kill this when parent dies
        t.start()
